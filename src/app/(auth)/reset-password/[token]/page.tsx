...
const ResetPasswordPage: React.FC = () => {
  ...
  const { token } = useParams() // Retrieve the token from the URL query parameters.
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-100">
        <div className="w-full max-w-md rounded bg-white p-8 text-center shadow-md">
          ...
          <p className="text-gray-700">The password reset link is invalid or expired. Please request a new one.</p>
        </div>
      </div>
    )
  }
        ...
        {message && (
          <div
            className={`mb-4 text-center ${
              message.includes("error") || message.includes("unexpected") ? "text-red-500" : "text-green-500"
            }`}
          >
            {message}
          </div>
